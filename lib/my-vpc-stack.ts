...
export class Vpc extends Construct {
    ...
    constructor(scope: Construct, id: string) {
        ...
        this.myVPC = new ec2.Vpc(this, `VPC`, {
            ...
            vpcName: `${CONSTANT.ENVIRONMENT.charAt(0) + CONSTANT.ENVIRONMENT.slice(1)}-vpc`,
        });
        ...
        for (const publicSubnet of this.myVPC.publicSubnets) {
            cdk.Aspects.of(publicSubnet).add(
                new cdk.Tag('Name', `${CONSTANT.ENVIRONMENT.charAt(0) + CONSTANT.ENVIRONMENT.slice(1)}-${publicSubnet.node.id.replace(/Subnet[0-9]$/, '')}-${publicSubnet.availabilityZone}`)
            );
            cdk.Aspects.of(publicSubnet).add(
        ...
        for (const privateSubnet of this.myVPC.privateSubnets) {
            cdk.Aspects.of(privateSubnet).add(
                new cdk.Tag('Name', `${CONSTANT.ENVIRONMENT.charAt(0) + CONSTANT.ENVIRONMENT.slice(1)}-${privateSubnet.node.id.replace(/Subnet[0-9]$/, '')}-${privateSubnet.availabilityZone}`)
            );
            cdk.Aspects.of(privateSubnet).add(
                new cdk.Tag('author', author)
            );
